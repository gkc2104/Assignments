...
object Anagrams {
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = {
    val temp = s.foldLeft("")( (x: String,y: String) => x ++ y)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ( for{
        (xc , xn) <- x
    } yield (xc, xn - y.toMap.getOrElse(xc,0))
        ).filter(_._2 !=0)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
        ...
        anagrams <- sentenceAnagrams( subtract(occurrences,wordOccurrences(word)) )
      } yield word :: anagrams
    }
